    ...
    real maxZ)
{
    ...
    if (coordNodeX >= minX && coordNodeX <= maxX && coordNodeY >= minY && coordNodeY <= maxY && coordNodeZ >= minZ &&
        coordNodeZ <= maxZ) {
        ...
        printf("Node %u \t (%f\t%f\t%f)\n rho: %f\t velo: %f\t %f \t %f\n\n", k_000, coordNodeX, coordNodeY, coordNodeZ,
               drho, vvx, vvy, vvz);
        printf("Node %u \t (%f\t%f\t%f)\n f_M00\t%f\t f_000\t%f\t f_P00\t%f\n f_MP0\t%f\t f_0P0\t%f\t f_PP0\t%f\n "
               ...
               k_000, coordNodeX, coordNodeY, coordNodeZ, f_M00, f_000, f_P00, f_MP0, f_0P0, f_PP0, f_MM0, f_0M0, f_PM0,
               f_M0P, f_00P, f_P0P, f_M0M, f_00M, f_P0M, f_MPP, f_0PP, f_PPP, f_MPM, f_0PM, f_PPM, f_MMP, f_0MP, f_PMP,
               f_MMM, f_0MM, f_PMM);
    }
}

void DistributionDebugInspector::inspect(Parameter* para, uint level, uint t)
{
    if (this->inspectionLevel != level)
        return;

    VF_LOG_INFO("{}: distributions on level {} at t {}", tag, level, t);

    vf::cuda::CudaGrid cudaGrid =
        vf::cuda::CudaGrid(para->getParD(level)->numberofthreads, para->getParD(level)->numberOfNodes);

    printFs<<<cudaGrid.grid, cudaGrid.threads>>>(
        para->getParD(level)->distributions.f[0], para->getParD(level)->isEvenTimestep,
